...
Topology (Phase 1):
  ...
  [company_researcher, contact_finder, signal_detector] → END   (fan-in)

The orchestrator node runs first and populates the shared LeadState fields
that the three subagent nodes each write to independently.  The fan-in edge
means all three must complete before the graph proceeds to END, giving the
parallel super-step behaviour required by CLAUDE.md.
"""
